...
def configure_gear_context(args):
    args.gear_feature_idx = None
    ...
    try:
        ...
        return
    if "gear_is_sled" in feature_columns:
        args.gear_feature_idx = feature_columns.index("gear_is_sled")
...
def test(args):
    ...
    configure_gear_context(args)
    test_idx = np.load(args.test_idx)
    if getattr(args, "use_db_loss", False):
        train_idx = np.load(args.train_idx)
        train_labels = get_label(data, train_idx, args.meta_offset, args.label_dim)
        args.class_freq_tensor = torch.from_numpy(train_labels.sum(0).astype(np.float32))
        args.train_sample_count = int(train_labels.shape[0])
